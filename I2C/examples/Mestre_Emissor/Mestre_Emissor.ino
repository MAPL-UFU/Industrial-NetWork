...
void loop() {
  ...
  */
  uint8_t packetBuffer[] = {(((digitalRead(pinBT)&3)<<4)|((digitalRead(pinA0)&3)<<2)|(digitalRead(pinA1)&3))};
  // uint8_t packetBuffer[] = {digitalRead(pinBT), digitalRead(pinA0), digitalRead(pinA1)};

  /* Envio da mensagem armazenada no buffer do mestre */
  Transmission(SLAVE, packetBuffer, sizeof(packetBuffer));
